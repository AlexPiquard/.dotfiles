	...
	while read line; do
		...
		if [ $count = 3 ]; then
			...
			if [[ "$theme" == "'prefer-dark'" ]]; then
				...
				echo "import = [ '~/.config/alacritty/tokyo-night.toml' ]" >> ~/.config/alacritty/auto-theme/theme.toml
			else
				echo "import = [ '~/.config/alacritty/github-light.toml' ]" >> ~/.config/alacritty/auto-theme/theme.toml
			fi
			count=0
		fi 
	done
