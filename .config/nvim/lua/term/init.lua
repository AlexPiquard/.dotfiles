...
local sizes = { sp = 0.3, vsp = 0.2, ["bo sp"] = 0.3, ["bo vsp"] = 0.2 }
...
M.display = function(opts)
  ...
  if (opts.pos == "sp" and not vim.g.nvhterm) or (opts.pos == "vsp" and not vim.g.nvvterm) or (opts.pos ~= "float") then
    ...
    local size = opts.size and opts.size or sizes[opts.pos]
  ...
  local winopts = vim.tbl_deep_extend("force", winopts, opts.winopts or {})
